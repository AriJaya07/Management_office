...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function fetchUser(): Promise<void> {
    try {
      const response = await $api<{ data: User }>('/api/v1/user')
  ...
  async function login(credentials: { email: string, password: string }): Promise<void> {
    const response = await $api<{ user: User }>('/api/v1/login', {
      method: 'POST',
      body: credentials,
    })
  ...
  async function register(payload: {
    ...
    const response = await $api<{ user: User }>('/api/v1/register', {
      method: 'POST',
      body: payload,
    })
    user.value = response.user
  ...
  return { user, isAuthenticated, hasRole, can, fetchUser, login, register, logout }
})
